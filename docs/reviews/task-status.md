...
| K03–K10 | **IN PROGRESS — CODE IMPLEMENTED** | Codex (sole active agent) | `codex/recorder-correctness` | PR #9: serialized lifecycle and generation-bound streams; effective/frozen settings; expiring GNSS validity; startup/stall/retry health; continuous disarming identity; persisted quality and verified badges; save retry/partial deletion; audio intent/stale expiry. Final native checks and merge pending. |
| K11 | **IN PROGRESS — CODE IMPLEMENTED** | Codex (sole active agent) | `codex/recorder-correctness` | PR #9: linear band crossings, gap barriers, stable interval IDs, smoothed chart/readout consistency and separate valid segments for angle and speed. |
| K12 | **IN PROGRESS — CODE IMPLEMENTED** | Codex (sole active agent) | `codex/recorder-correctness` | Specification merged in PR #4. PR #9 implements explicit-unit analytical model, calibration-bias component since anchor, metadata and gap invalidation. Physical accuracy/coverage cannot be certified remotely. |
| K13 | **IN PROGRESS — CODE IMPLEMENTED** | Codex (sole active agent) | `codex/recorder-correctness` | PR #9 implements opt-in diagnostics, Wi-Fi-only transfers, coordinate-redacted copies, post-consent files only and revocation cancellation; privacy documents updated. Verified publisher contact remains required for publication. |
...
| K15 | **IN PROGRESS — CODE IMPLEMENTED** | Codex (sole active agent) | `codex/recorder-correctness` | PR #9 bounds idle sample retention and caches immutable history statistics. Query timing and retained-sample checks pending. Eager history loading remains; no startup-memory or lazy-loading improvement is claimed. |
| K16 | **IN PROGRESS — CODE IMPLEMENTED** | Codex (sole active agent) | `codex/recorder-correctness` | PR #9 implements truthful live status, unsaved-attempt retry, quality explanation and per-run JSON sharing. Corrected meter artwork preserved; speculative features remain product backlog. |
